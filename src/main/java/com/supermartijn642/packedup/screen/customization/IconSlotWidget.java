...
public class IconSlotWidget extends BaseWidget {
    ...
    public IconSlotWidget(int x, int y, EnumHand hand, Supplier<ItemStack> backpack, Runnable selectionScreen){
        super(x, y, 20, 20);
        this.hand = hand;
    ...
    public void renderBackground(int mouseX, int mouseY){
        ...
        ScreenUtils.drawTexture(this.x, this.y, this.width, this.height, this.isFocused() ? 0.5f : 0, 0, 0.5f, 1);
    }

    @Override
    public void render(int mouseX, int mouseY){
        super.render(mouseX, mouseY);
        ClientUtils.getItemRenderer().renderItemIntoGUI(this.backpackSupplier.get(), this.x + 2, this.y + 2);
    }
}
