    ...
    public static void load(){
        ...
        for(Map.Entry<Integer,BackpackInventory> entry : inventories.entrySet()){
            ...
            for(ItemStack stack : inventory.getStacks()){
                if(stack.getItem() instanceof BackpackItem && stack.hasTagCompound() && stack.getTagCompound().hasKey("packedup:invIndex")){
                    int index = stack.getTagCompound().getInteger("packedup:invIndex");
                    if(!inventories.containsKey(index)){
                        stack.getTagCompound().removeTag("packedup:invIndex");
                        continue;
                    }
