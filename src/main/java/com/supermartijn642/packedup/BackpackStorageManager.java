...
public class BackpackStorageManager {
    ...
    public static void onLevelSave(WorldEvent.Save event){
        if(event.getWorld().isClientSide() || !(event.getWorld() instanceof World) || (((World)event.getWorld()).dimension() != World.OVERWORLD && System.currentTimeMillis() - lastSaveTimestamp < 30000))
            return;
        save();
    }
