...
public class BackpackItemRenderer implements CustomItemRenderer {
    ...
    public void render(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack poseStack, MultiBufferSource bufferSource, int combinedLight, int combinedOverlay){
        ...
        // Render the icon
        if(transformType == ItemTransforms.TransformType.GUI && !RECURSION_GUARD.get()){
            ...
            if(!icon.isEmpty()){
                ...
                ClientUtils.getItemRenderer().renderStatic(icon, ItemTransforms.TransformType.GUI, combinedLight, combinedOverlay, poseStack, bufferSource, 0);
                poseStack.popPose();
            }
